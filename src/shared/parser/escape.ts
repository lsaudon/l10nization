class StringEscapeSequence {
  ...
  getUnescapedString = (input: string): string => (input.match(this.unescapedStringRegex) ?? [])[1].replace(/\\n/gu, '\n');
}

export const escapeSequences = ['r"""', "r'''", 'r"', "r'", '"""', "'''", '"', "'"].map((start) => new StringEscapeSequence(start));
